...
import { SportDbPlayerSearchResult } from '../../../shared/models/sportdb-football';
import {
  FormSelectionConfig,
  FormSelectionOption,
  ...
  template: `
    ...
    @if (showResultsPanel()) {
      <div class="results-panel">
        @for (option of playerOptions; track option.value) {
          <button
            type="button"
            class="result-option"
      ...
      <div class="results-panel">
        <p class="panel-message">No matching players found.</p>
      </div>
    }
  `,
})
export class PlayerAutocompleteComponent {
  ...
  readonly playerControl = new FormControl<FormSelectionOption | string | null>(null);
  ...
  constructor() {
    effect(() => {
      ...
      if (fieldConfig.disabled) {
        if (this.playerControl.enabled) {
          this.playerControl.disable({ emitEvent: false });
        }
      ...
      if (this.playerControl.disabled) {
        this.playerControl.enable({ emitEvent: false });
      }
    });
  }

  private readonly searchTerm$ = this.playerControl.valueChanges.pipe(
    startWith(this.playerControl.value),
    map(value => {
  ...
  readonly hintText = computed(() => {
    if (this.config().disabled) {
      return 'Select two teams first to enable player search.';
    }

    if (this.selectedOption()) {
      return 'Player selected from live API results.';
    }

    return this.hasSearchTerm()
      ? 'Select a player from the API results below.'
      : `Type at least ${MIN_SEARCH_LENGTH} characters to search players.`;
  });
  ...
  readonly options = toSignal(
    this.searchTerm$.pipe(
      switchMap(searchTerm => {
        ...
        return this.sportDbFootballService.searchPlayers(searchTerm).pipe(
          tap(response => {
            console.log(
              `[PlayerAutocomplete] First results for "${searchTerm}"`,
              response.results.slice(0, 10),
            );
          }),
          map(response => response.results.map(player => this.mapPlayerToOption(player))),
          catchError(error => {
            console.error('[PlayerAutocomplete] Player search failed', error);
            this.errorMessage.set('Player search is blocked in the browser. Restart `npm start` to use the local proxy.');
            return of([] as readonly FormSelectionOption[]);
          }),
  ...
  selectOption(option: FormSelectionOption): void {
    ...
    this.playerControl.setValue(option.label, { emitEvent: false });
    this.selectedOptionChange.emit(option);
    this.isInputFocused.set(false);
  }

  private clearSelection(): void {
    const currentControlValue = this.playerControl.value;
  ...
  private mapPlayerToOption(player: SportDbPlayerSearchResult): FormSelectionOption {
    const clubSuffix = player.club?.name ? ` - ${player.club.name}` : '';
    const positionSuffix = player.position ? ` (${player.position})` : '';

    return {
      value: player.id,
      label: `${player.name}${positionSuffix}${clubSuffix}`,
    };
  }
}
